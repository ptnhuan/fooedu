...
use Contact\Controllers\Admin\ContactController;
use URL;
use Route,
    Redirect;
use Contact\Models\Contacts;
/**
 * Validators
 */
use Contact\Validators\ContactAdminValidator;
...
class ContactAdminController extends ContactController {
        ...
        if (!$this->obj_validator->validate($input)) {
            ...
            if (!empty($contact_id) && is_int($contact_id)) {
                ...
                if (!empty($contact)) {
                    ...
                    //Message
                    \Session::flash('message', trans('contact::contact.message_update_successfully'));
                    return Redirect::route("admin_contact.edit", ["id" => $contact->contact_id]);
                } else {

                    //Message
                    \Session::flash('message', trans('contact::contact.message_update_unsuccessfully'));
                }
            } else {
                ...
                if (!empty($contact)) {
                    ...
                    //Message
                    \Session::flash('message', trans('contact::contact.message_add_unsuccessfully'));
                }
            }
        }
    ...
    public function delete(Request $request) {
        ...
        if (!empty($contact_id)) {
            ...
            if (!empty($contact)) {
                //Message
                \Session::flash('message', trans('contact::contact.message_delete_successfully'));
